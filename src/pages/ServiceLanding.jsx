import { ChevronDown, MessageCircle, ShieldCheck, Sparkles, Snowflake, MonitorPlay, BatteryCharging, Armchair, Headphones, SprayCan, CalendarDays } from 'lucide-react';
...
export default function ServiceLanding(){
 ...
 return <main className="service-page">
  ...
  {data.featureDetails&&<section className="service-section service-soft"><div className="content"><div className="section-heading"><span className="eyebrow">TOP FEATURES</span><h2>Comfort, convenience and a better group journey.</h2><p>Explore the features designed to make every Tempo Traveller ride more comfortable and practical.</p></div><div className="service-feature-grid">{data.featureDetails.map((x,i)=>{const Icon=featureIcons[i%featureIcons.length];return <article key={x[0]}><Icon/><h3>{x[0]}</h3><p>{x[1]}</p></article>})}</div></div></section>}
