...
export default function Fleet(){
  return <Page eyebrow="FLEET & RENTALS" title="Choose the right vehicle for your journey." text="From economical Tirupati cabs to premium Urbania, Tempo Travellers and large buses, choose the space, comfort and price point that fits your trip." image={images.hero}>
    <section className="content" id="rentals">
      ...
      {fleetCategories.slice(1).map(category=>{const items=fleet.filter(v=>category.ids.includes(v.id));return <section className="fleet-group" id={category.key} key={category.key}><div className="fleet-group-heading"><div><span className="eyebrow">{category.key==='cars'?'CARS':category.key==='tempo'?'TEMPO TRAVELLERS':category.key==='urbania'?'PREMIUM GROUP TRAVEL':'LARGE GROUP TRAVEL'}</span><h3>{category.label}</h3>{category.key==='cars'&&<p className="fleet-category-note">Available for local travel in Tirupati, Tirupati Airport transfers, and outstation trips.</p>}</div><span>{items.length} options</span></div><div className="vehicle-slider">{items.map(v=><article className="rental-card" key={v.id}>
